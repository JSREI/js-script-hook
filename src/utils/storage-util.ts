...
function diagnoseGMStorageAPI(): { getValue: (key: string, defaultValue?: any) => any; setValue: (key: string, value: any) => void } {
    console.log('[存储诊断] 开始诊断油猴存储API...');
    
    // 创建最终返回的API对象
    const api = {
        getValue: (key: string, defaultValue?: any): any => {
            console.error('[存储] 无法获取值, 存储API不可用');
            return defaultValue;
        },
        setValue: (key: string, value: any): void => {
            console.error('[存储] 无法保存值, 存储API不可用');
        }
    };
    ...
    try {
        ...
        if (typeof GM_setValue === 'function' && typeof GM_getValue === 'function') {
            console.log('[存储诊断] 检测到标准GM_setValue/GM_getValue API');
        ...
        api.setValue = (key: string, value: any): void => {
            try {
                if (typeof GM_setValue === 'function') {
                    ...
                    console.log(`[存储] 尝试使用GM_setValue保存 ${key}`);
                    return;
                }
                ...
                if (typeof GM !== 'undefined' && GM && typeof GM.setValue === 'function') {
                    try {
                        ...
                        if (result !== undefined && result !== null && typeof result === 'object' && 'then' in result) {
                            ...
                            promiseResult.catch(err => console.error('[存储] 异步保存失败:', err));
                        }
                        console.log(`[存储] 尝试使用GM.setValue保存 ${key}`);
                        return;
                    } catch (e) {
                        console.error('[存储] GM.setValue调用失败:', e);
                    }
                }
                
                // 由于我们当前环境不是标准的油猴环境，请直接使用油猴API
                console.error('[存储] 所有已知存储API尝试失败，无法保存配置');
            } catch (error) {
                console.error('[存储] 保存时发生未知错误:', error);
            }
        };
        ...
        api.getValue = (key: string, defaultValue?: any): any => {
            try {
                if (typeof GM_getValue === 'function') {
                    ...
                    console.log(`[存储] 尝试使用GM_getValue读取 ${key}:`, value);
                ...
                if (typeof GM !== 'undefined' && GM && typeof GM.getValue === 'function') {
                    try {
                        ...
                        // 只有当result不是undefined或null时，才检查它是否是Promise
                        if (result !== undefined && result !== null && typeof result === 'object' && 'then' in result) {
                            console.warn(`[存储] GM.getValue返回Promise，无法同步读取 ${key}`);
                            return defaultValue;
                        }
                        console.log(`[存储] 尝试使用GM.getValue读取 ${key}:`, result);
                        return result ?? defaultValue;
                    } catch (e) {
                        console.error('[存储] GM.getValue调用失败:', e);
                        return defaultValue;
                    }
                }
                
                console.error('[存储] 所有已知存储API尝试失败，无法读取配置');
                return defaultValue;
            } catch (error) {
                console.error(`[存储] 读取${key}时发生未知错误:`, error);
                return defaultValue;
            }
        };
    } catch (error) {
        console.error('[存储诊断] 诊断过程发生未知错误:', error);
    }
    
    console.warn('[存储诊断] 未能找到可靠的存储API，将使用有限功能的回退方式');
    return api;
}
...
export function saveValue(key: string, value: any): void {
    try {
        ...
        if (typeof value === 'object') {
            ...
            console.warn(`[存储] 保存的值为undefined，使用null替代`);
            value = null;
        }
        
        console.log(`[存储] 正在保存配置 ${key}:`, value);
        ...
        try {
            ...
            if (savedValue !== value && JSON.stringify(savedValue) !== JSON.stringify(value)) {
                console.warn(`[存储] 保存验证不一致: 期望=${value}, 实际=${savedValue}`);
            }
        } catch (verifyError) {
            console.error('[存储] 保存验证失败:', verifyError);
        }
    } catch (error) {
        console.error(`[存储] 保存 ${key} 失败:`, error);
    }
}
        ...
        if (typeof value === 'string') {
            try {
                ...
                if ((value.startsWith('{') && value.endsWith('}')) || 
                    ...
                    console.log(`[存储] 成功将 ${key} 解析为对象:`, parsed);
                    return parsed;
                }
            } catch (jsonError) {
                // 解析失败，返回原始字符串
                console.log(`[存储] ${key} 不是JSON格式，返回原始字符串`);
            }
        }
        
        console.log(`[存储] 读取完成 ${key}:`, value);
